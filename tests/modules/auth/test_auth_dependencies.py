...
@pytest.mark.asyncio
async def test_get_current_user_passthrough(user_factory: Callable[..., UserStub]) -> None:
    user = user_factory()
...
@pytest.mark.asyncio
async def test_require_groups_raises_403_when_groups_missing(guest_user: UserStub) -> None:
...
@pytest.mark.asyncio
async def test_require_permission_raises_403_when_permission_missing(guest_user: UserStub) -> None:
    dependency = require_permission("spec:generate")
